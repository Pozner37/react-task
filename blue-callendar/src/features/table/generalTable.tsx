...
function GeneralTable() {
    ...
    const taskRows = () => tasks.map((task: CalendarTask) => (
        <GeneralTableRow key={task.getId()} item={task} typeIcon={getTaskTypeIcon(task)} priorityIcon={getTaskPriorityIcon(task)}
                         other={<TaskOther task={task}/>}
                         collapseBody={<TaskCollapseBody task={task}/>}></GeneralTableRow>));
